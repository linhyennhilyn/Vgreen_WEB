...
/**
 * Tự động cập nhật status của sản phẩm dựa trên stock
 * - Nếu stock = 0 và status = "Active" → chuyển sang "OutOfStock"
 * - Nếu stock > 0 và status = "OutOfStock" → chuyển về "Active"
 * @param {string} sku - SKU của sản phẩm
 * @param {number} newStock - Stock mới sau khi cập nhật
 */
async function updateProductStatusByStock(sku, newStock) {
  try {
    const product = await Product.findOne({ sku: sku });
    if (!product) {
      console.warn(`⚠️ [Stock Status] Product not found for SKU: ${sku}`);
      return;
    }
    ...
    if (newStock <= 0 && currentStatus === "Active") {
      ...
      console.log(
        `📦 [Stock Status] Product ${sku} status changed: ${currentStatus} → ${newStatus} (stock: ${newStock})`
      );
    }
    // Nếu stock > 0 và status = "OutOfStock" → chuyển về "Active"
    ...
    console.error(
      `❌ [Stock Status] Error updating product status for SKU ${sku}:`,
      error
    );
