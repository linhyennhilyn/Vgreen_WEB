...
app.post("/api/auth/reset-password", checkMongoConnection, async (req, res) => {
  try {
    ...
    if (email && otp && newPassword) {
      ...
      // Bước 5: Lưu password mới dưới dạng plain text (không hash) cho admin
      // console.log("🔐 Saving new password as plain text for admin...");
      // COMMENTED: Không hash password cho admin, giữ nguyên plain text
      // const saltRounds = 10;
      // const hashedPassword = await bcrypt.hash(newPassword, saltRounds);
      // console.log("✅ Password hashed successfully");

      // Bước 6: Cập nhật password mới (plain text, không hash) và xóa OTP
      // console.log("🔄 Updating password and clearing OTP...");
      await collection.updateOne(
        { email: email },
        {
          $set: {
            password: newPassword, // Lưu plain text, không hash cho admin
            updated_at: new Date(),
          },
          $unset: {
            reset_otp: "",
...
app.get("/api/orders", checkMongoConnection, async (req, res) => {
  try {
    ...
    if (CustomerID) {
      ...
      // COMMENTED: Tự động xóa sản phẩm inactive khỏi đơn hàng pending/processing khi load
